...
export async function POST(req: NextRequest) {
	...
	return await establishconnection().then(async () => {
        ...
        const discount = rqst.discount;
        const amountreceived = rqst.amountreceived;
        const userID = rqst.orderMadeBy.userID;
        const deviceID = rqst.orderMadeBy.deviceID;
        const isRenewed = rqst.isRenewed;
        return await Order.updateOne({ orderID: orderID, "orderMadeBy.userID": userID, "orderMadeBy.deviceID": deviceID }, { receivedAmount: amountreceived, discount: discount, status: isRenewed ? "Renewed" : "Initial" }).then(async () => {
